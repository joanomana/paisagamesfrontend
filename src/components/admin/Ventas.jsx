...
async function getProduct(productId) {
    if (!productId)
        return { _id: "", nombre: "Producto", imagen: "/placeholder.png" };
...
export default function Ventas() {
    ...
    // ⬇️ Callback que actualiza en servidor y sincroniza el estado del padre
    const onUpdateEstado = async (ventaId, nuevoEstado) => {
        try {
            const updated = await ventasAPI.update(ventaId, { estado: nuevoEstado });
            setVentas((prev) =>
                prev.map((v) =>
                    v._id === ventaId ? { ...v, estado: updated.estado } : v
                )
            );
            return { ok: true, estado: updated.estado };
        } catch (e) {
        ...
        <div className="space-y-5 text-white">
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="rounded-2xl border border-white/15 bg-white/5 px-4 py-2 text-sm">
                    ...
                    <span className="font-semibold text-white">{totalPendientes}</span>
                    <br />
            ...
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                {ventas.map((v) => (
                    <VentaCard key={v._id} venta={v} onUpdateEstado={onUpdateEstado} />
                ))}
            </div>
        </div>
    );
...
function VentaCard({ venta, onUpdateEstado }) {
    ...
    const guardarEstado = async () => {
        if (!editing) return;
        setSaving(true);
        const res = await onUpdateEstado(venta._id, nuevoEstado); // ← llama al padre
        setSaving(false);
        if (res.ok) {
            setEditing(null);
            setErr("");
            ...
            <header className="mb-4 flex flex-col items-start justify-between gap-3">
                <div className="min-w-0">
                    <div className="text-xs text-white/50">
                        #{String(venta._id).slice(-8)} • {fecha}
                    </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
                        {venta.metadatos?.metodoPago && (
                            <Chip>{venta.metadatos.metodoPago}</Chip>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col items-start gap-2">
                    ...
                    {editing && (
                        <div className="mt-3 rounded-2xl border border-white/15 p-5 text-white shadow-xl bg-[#0b1220]">
                            <div className="space-y-2 text-sm">
                                
                                <label
                                    className="block text-white/80"
                                    htmlFor={`estado-select-${venta._id}`}
                                >
                                    Estado
                                </label>
                                <select
                                    id={`estado-select-${venta._id}`}
                                    value={nuevoEstado}
                            ...
                            <div className="mt-4 flex justify-end gap-2">
                                ...
                                <button
                                    ...
                                    className="rounded-lg bg-white px-3 py-2 text-sm font-medium text-[#0b1220] hover:bg-gray-100 disabled:opacity-60"
                                    disabled={saving}
                                >
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-2 2xl:grid-cols-3">
                {(venta.items || []).map((it, idx) => {
                    ...
                    return (
                        <div
                            key={key}
                            className="rounded-xl border border-white/10 bg-[#0b1220]/60 p-3"
                        >
                            <div className="aspect-square w-full overflow-hidden rounded-lg bg-black/20 ring-1 ring-white/10">
                                <img
                                    src={
                                        it._producto?.imagen ||
                                        it._producto?.imagenes?.[0] ||
                                        "/placeholder.png"
                                    }
                                    alt={it._producto?.nombre || "Producto"}
                            ...
                            <div className="mt-2">
                                <div className="truncate text-sm font-medium">
                                    {it._producto?.nombre || "Producto"}
                                </div>
                                <div className="mt-1 flex flex-col items-center gap-0.5 text-xs text-white/70">
                                    <span>
                                        {it.cantidad} × {COP.format(it.precioUnitario || 0)}
                                    </span>
                                    <span className="font-semibold text-white">
                                        {COP.format(it.subtotal || 0)}
                                    </span>
                                </div>
                            </div>
                        </div>
...
function EstadoBadge({ estado }) {
    ...
    return (
        <span
            className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${cls}`}
        >
            {estado || "—"}
        </span>
    );
